...
@interface Rapidops : NSObject

#pragma mark - Core

/**
 * Returns @c Rapidops singleton to be used throughout the app.
 * @return The shared @c Rapidops object
 */
+ (instancetype)sharedInstance;

/**
 * Starts Rapidops with given configuration and begins session.
 * @param config @c RapidopsConfig object that defines host, app key, optional features and other settings
 */
 ...
 * @param onServer If set, data on Rapidops Server will be merged automatically, otherwise device will be counted as a new device
 */
...
#if TARGET_OS_WATCH
/**
 * Suspends Rapidops, adds recorded events to request queue and ends current session.
 * @discussion This method needs to be called manually only on @c watchOS, on other platforms it will be called automatically.
 */
- (void)suspend;

/**
 * Resumes Rapidops, begins a new session after the app comes to foreground.
 * @discussion This method needs to be called manually only on @c watchOS, on other platforms it will be called automatically.
 */
- (void)resume;
#endif
...
/**
 * Records push notification token to Rapidops Server for current device ID.
...
/**
 * Clears push notification token on Rapidops Server for current device ID.
 ...
 * @discussion By default, Rapidops Server uses a geo-ip database for acquiring user's location.
 ...
 * @discussion Logs recorded by this method are stored in a non-persistent structure, and delivered to Rapidops Server only in case of a crash.
...
/**
 * Returns @c RapidopsUserDetails singleton to be used throughout the app.
 * @return The shared @c RapidopsUserDetails object
 */
 ...
 * @discussion This is just a convenience method that handles setting user ID as new device ID and merging existing data on Rapidops Server.
 ...
 * @discussion If user dismissed dialog without giving a rating, this value will be 0 and it will not be reported to Rapidops Server.
 ...
 * @param widgetID ID of the feedback widget created on Rapidops Server.
 ...
 * @discussion If remote config is never retrieved from Rapidops Server before, this method will return @c nil.
 * @discussion If @c key is not defined in remote config on Rapidops Server, this method will return @c nil.
 * @discussion If Rapidops Server is not reachable, this method will return the last retrieved value which is stored on device.
 * @param key Remote config key specified on Rapidops Server
 */
- (id)remoteConfigValueForKey:(NSString *)key;

/**
 * Manually updates all locally stored remote config values by fetching latest values from Rapidops Server, and executes completion handler.
...
/**
 * Manually updates locally stored remote config values only for specified keys, by fetching latest values from Rapidops Server, and executes completion handler.
 * @discussion @c completionHandler has an @c NSError parameter that will be either @ nil or an @c NSError object, depending on result.
...
/**
 * Manually updates locally stored remote config values except for specified keys, by fetching latest values from Rapidops Server, and executes completion handler.
 * @discussion @c completionHandler has an @c NSError parameter that will be either @ nil or an @c NSError object, depending on result.
